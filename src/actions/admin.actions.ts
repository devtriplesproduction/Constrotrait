...
import { updateEmployeeProfileSchema } from "@/lib/validations/employee";
...
export async function toggleUserActiveAction(userId: string, isActive: boolean) {
  ...
  const result = await toggleUserActive(userId, isActive, user);
  if (result.success) {
...
export async function offboardEmployeeAction(userId: string) {
  ...
  const result = await offboardEmployee(userId, user);
  if (result.success) {
    revalidatePath("/employees", "page");
  }
  return result;
...
export async function onboardEmployeeAction(userId: string) {
  ...
  const result = await onboardEmployee(userId, user);
  if (result.success) {
    revalidatePath("/employees", "page");
  }
  return result;
...
export async function deleteEmployeeAction(userId: string) {
  ...
  const result = await deleteEmployee(userId, user);
  if (result.success) {
    revalidatePath("/employees", "page");
  }
  return result;
...
export async function updateEmployeeProfileAction(userId: string, data: Record<string, unknown>) {
  ...
  const result = await updateEmployeeProfile(userId, parsedData.data);
  if (result.success) {
    revalidatePath("/employees", "page");
  }
  return result;
