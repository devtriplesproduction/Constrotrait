...
export async function generateSalarySlipAction(snapshotId: string, month: number, year: number) {
  try {
    ...
    const { data: snapshot, error: snapshotError } = await supabaseAdmin
      ...
      .select('id, employee_id, cycle_id, employee_name, employee_id_external, department, designation, days_present, days_paid_leave, days_absent, days_unpaid_leave, basic_salary, pf, hra, esi, allowance, professional_tax, bonus, income_tax, overtime_pay, other_deductions, damage_recovery, salary_advance_recovery, gross_salary, total_deductions, net_salary, profiles!payroll_snapshots_employee_id_fkey(branch_id)')
      .eq('id', snapshotId)
      .single();
...
export async function emailSalarySlipAction(snapshotId: string) {
  try {
    ...
    // Mock Email Send removed: We do not have an email provider configured.
...
export async function downloadSalarySlipAction(employeeId: string, month: number, year: number) {
  try {
    const user = await getAuthenticatedUserWithRoles();
    if (!user || (!isHR(user.roles) && !isSuperAdmin(user.roles) && !isBranchManager(user.roles))) {
      return { success: false, error: "Unauthorized access." };
