...
export function PayrollClient({
  ...
  const handleAddAdjustment = async (e: React.FormEvent) => {
    ...
    if (!adjEmployeeId || isNaN(finalAmount) || finalAmount <= 0) {
  ...
  const handleDownloadSlip = async (employeeId: string) => {
    setActionLoading(true);
    try {
      const res = await downloadSalarySlipAction(employeeId, month, year);
      if (res.success && res.signedUrl) {
        window.open(res.signedUrl, '_blank');
      } else {
                          ...
                          data.map((emp) => (
                            <tr key={emp.employee_id} className="hover:bg-orange-50/30 transition-colors">
                              <td className="px-6 py-4">
                                ...
                                <div className="text-xs text-slate-500 font-mono mt-0.5 w-32 truncate" title={emp.employee_id}>{emp.employee_id}</div>
                              </td>
                          ...
                          {data.length === 0 ? (
                            ...
                            data.map((emp) => (
                              <tr key={emp.employee_id} className="hover:bg-orange-50/30 transition-colors">
                                <td className="px-6 py-4">
                                  <div className="font-bold text-slate-900">{emp.employee_name}</div>
                                  <div className="text-xs text-slate-500 font-mono mt-0.5 max-w-[120px] truncate" title={emp.employee_id}>{emp.employee_id}</div>
                                </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white/90 backdrop-blur-2xl border border-white/80 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col">
            ...
            <form onSubmit={handleAddAdjustment} className="px-6 py-6 overflow-y-auto space-y-4">
              ...
              {(adjType !== "Travel Expense" || adjVehicleType === "Car") && adjType !== "Food Allowance" && (
                <div>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-x-8 gap-y-4">
                    <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                      ...
                      <span className="font-semibold">{selectedEmployee.base_salary != null ? `₹${selectedEmployee.base_salary.toLocaleString()}` : '-'}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                      <span className="text-sm font-medium text-muted-foreground">Basic Salary</span>
                      <span className="font-semibold">{selectedEmployee.basic_salary != null ? `₹${selectedEmployee.basic_salary.toLocaleString()}` : '-'}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                      <span className="text-sm font-medium text-muted-foreground">HRA</span>
                      <span className="font-semibold">{selectedEmployee.hra != null ? `₹${selectedEmployee.hra.toLocaleString()}` : '-'}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                      <span className="text-sm font-medium text-muted-foreground">Allowance</span>
                      <span className="font-semibold">{selectedEmployee.allowance != null ? `₹${selectedEmployee.allowance.toLocaleString()}` : '-'}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                      <span className="text-sm font-medium text-muted-foreground">Bonus</span>
                      <span className="font-semibold">{selectedEmployee.bonus != null ? `₹${selectedEmployee.bonus.toLocaleString()}` : '-'}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                      <span className="text-sm font-medium text-muted-foreground">Medical Allowance</span>
                      <span className="font-semibold">{selectedEmployee.medical_allowance != null ? `₹${selectedEmployee.medical_allowance.toLocaleString()}` : '-'}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                      <span className="text-sm font-medium text-muted-foreground">Travel Expense</span>
                      <span className="font-semibold">{selectedEmployee.travel_expense != null ? `₹${selectedEmployee.travel_expense.toLocaleString()}` : '-'}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                      <span className="text-sm font-medium text-muted-foreground">Perf. Incentive</span>
                      <span className="font-semibold">{selectedEmployee.performance_incentive != null ? `₹${selectedEmployee.performance_incentive.toLocaleString()}` : '-'}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                      <span className="text-sm font-medium text-muted-foreground">Food Allowance</span>
                      <span className="font-semibold">{selectedEmployee.food_allowance != null ? `₹${selectedEmployee.food_allowance.toLocaleString()}` : '-'}</span>
                    </div>
                  </div>
                  <div className="mt-4 pt-3 flex justify-between items-center bg-emerald-50/50 p-3 rounded-xl border border-emerald-100">
                    <span className="text-sm font-bold text-emerald-800">Gross Salary</span>
                    <span className="text-lg font-black text-emerald-600">{selectedEmployee.gross_salary != null ? `₹${selectedEmployee.gross_salary.toLocaleString()}` : '-'}</span>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-x-8 gap-y-4">
                    <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                      ...
                      <span className="font-semibold">{selectedEmployee.pf != null ? `₹${selectedEmployee.pf.toLocaleString()}` : '-'}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                      <span className="text-sm font-medium text-muted-foreground">ESI</span>
                      <span className="font-semibold">{selectedEmployee.esi != null ? `₹${selectedEmployee.esi.toLocaleString()}` : '-'}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                      <span className="text-sm font-medium text-muted-foreground">Prof. Tax</span>
                      <span className="font-semibold">{selectedEmployee.professional_tax != null ? `₹${selectedEmployee.professional_tax.toLocaleString()}` : '-'}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                      <span className="text-sm font-medium text-muted-foreground">TDS</span>
                      <span className="font-semibold">{selectedEmployee.tds != null ? `₹${selectedEmployee.tds.toLocaleString()}` : '-'}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                      <span className="text-sm font-medium text-muted-foreground">Salary Advance</span>
                      <span className="font-semibold">{selectedEmployee.salary_advance_recovery != null ? `₹${selectedEmployee.salary_advance_recovery.toLocaleString()}` : '-'}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                      <span className="text-sm font-medium text-muted-foreground">Damage Recovery</span>
                      <span className="font-semibold">{selectedEmployee.damage_recovery != null ? `₹${selectedEmployee.damage_recovery.toLocaleString()}` : '-'}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                      <span className="text-sm font-medium text-muted-foreground">Other</span>
                      <span className="font-semibold">{selectedEmployee.other_deductions != null ? `₹${selectedEmployee.other_deductions.toLocaleString()}` : '-'}</span>
                    </div>
                  </div>
                  <div className="mt-4 pt-3 flex justify-between items-center bg-rose-50/50 p-3 rounded-xl border border-rose-100">
                    <span className="text-sm font-bold text-rose-800">Total Deductions</span>
                    <span className="text-lg font-black text-rose-600">{selectedEmployee.total_deductions != null ? `-₹${selectedEmployee.total_deductions.toLocaleString()}` : '-'}</span>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-6">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <span className="text-4xl font-black tracking-tight">{selectedEmployee.net_salary != null ? `₹${selectedEmployee.net_salary.toLocaleString()}` : '-'}</span>
                    </div>
