...
export function OnboardForm({ onSuccess }: OnboardFormProps) {
  ...
  useEffect(() => {
    if (firstName && lastName) {
      ...
      if (safeFirst || safeLast) {
        const parts = [safeFirst, safeLast].filter(Boolean);
        const generatedEmail = `${parts.join('.')}@constrotrait.com`;
    ...
    if (step === 4) {
      async function fetchPreview() {
        try {
          ...
          if (!error && data) {
            setValue("employee_id", data as string, { shouldValidate: true });
          }
        } catch (e) {
          console.error("Failed to preview employee ID", e);
        }
      }
      fetchPreview();
    }
  }, [step, watchedBranchId, isSuperAdmin, setValue]);
  ...
  const onSubmit = async (data: OnboardFormData) => {
    ...
    const uploadedDocs: { id: string; name: string; size: number; path: string; uploaded_at: string; type: string }[] = [];
    let uploadedAvatarPath = "";
    
    try {
      const { uploadEmployeeFileAction } = await import("@/actions/employee.actions");
      ...
      if (selectedAvatarFile) {
        ...
        if (!uploadRes.success) {
           throw new Error(`Profile Photo Upload Failed: ${uploadRes.error}`);
        }
        uploadedAvatarPath = uploadRes.path as string;
      }

      // Upload documents to Supabase Storage via Server Action
      for (const f of uploadedFiles) {
        if (f.file) {
          ...
          if (!uploadRes.success) {
             throw new Error(`Failed to upload ${finalName}: ${uploadRes.error}`);
          }
      ...
      if (result?.success) {
        ...
        if (uploadedDocs.length > 0) {
           await supabase.storage.from('employee-documents').remove(uploadedDocs.map(d => d.path));
        }
        if (uploadedAvatarPath) {
           await supabase.storage.from('employee-documents').remove([uploadedAvatarPath]);
        }
        toast({
      ...
      toast({
        ...
        description: err instanceof Error ? err.message : "An unexpected network or file engine error occurred.",
        variant: "error"
      });
    } finally {
