...
import { approveLeaveAction, rejectLeaveAction, cancelApprovedLeaveAction, verifyMedicalCertificateAction, deleteMedicalCertificateAction, rejectMedicalCertificateAction, getMedicalCertificateUrlAction } from "@/actions/leave.actions";
import { useToast } from "@/hooks/use-toast";
import { format } from "date-fns";
import { Calendar, Clock, CheckCircle2, XCircle, AlertCircle, FileText, UserCircle2, Plus } from "lucide-react";
...
export function LeaveClientPage({ myLeaves, canApprove, leavesToApprove, isHR, compOffBalance, isSuperAdmin = false }: {
  ...
  const handleUploadAndVerify = useCallback(async () => {
    ...
    try {
      ...
      const { uploadMedicalCertificateAction } = await import("@/actions/leave.actions");
      const uploadRes = await uploadMedicalCertificateAction(uploadingCertFor, formData);
      if (!uploadRes.success || !('path' in uploadRes) || !uploadRes.path) {
        throw new Error(uploadRes.error || "Failed to upload medical certificate");
      }
      setUploadProgress(100);
      const res = await verifyMedicalCertificateAction(uploadingCertFor, uploadRes.path);
      if (res.success) toast({ title: "Success", variant: "success" });
      else toast({ title: "Error", description: res.error, variant: "error" });
      setUploadingCertFor(null);
      setCertFile(null);
    } catch (e: unknown) {
      console.error(e);
      toast({ title: "Error", description: e instanceof Error ? e.message : "Failed to process certificate", variant: "error" });
    } finally {
      setUploading(false);
      setTimeout(() => setUploadProgress(0), 500);
    }
  }, [uploadingCertFor, certFile, toast]);

  const { prompt, PromptComponent } = usePrompt();

  const handleViewMedicalCertificate = async (leaveId: string) => {
    const res = await getMedicalCertificateUrlAction(leaveId);
    if (res.success && res.url) setViewingCertUrl(res.url);
    else toast({ title: "Error", description: res.error, variant: "error" });
  };

  type LeaveAction = (id: string, extra: string) => Promise<{ success: boolean; error?: string }>;

  const handleAction = async (action: LeaveAction, id: string, extra?: string) => {
    try {
        ...
        <div className="space-y-6">
          ...
          {filteredMyLeaves.length === 0 && !showForm ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredMyLeaves.map((leave: LeaveRecord & { is_paid?: boolean }, idx) => {
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between relative z-10">
                      ...
                      {leave.status === 'Approved' && isHR && (
                        <Button variant="outline" size="sm" className="rounded-xl text-red-600 hover:text-red-700 hover:bg-red-50 border-red-200 h-8" onClick={() => handleAction(cancelApprovedLeaveAction, leave.id as string)}>
                          Cancel
                        </Button>
                      )}
                    </div>
        ...
        <div className="space-y-6">
          {filteredLeavesToApprove.length === 0 ? (
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              {filteredLeavesToApprove.map((leave: LeaveRecord & { is_paid?: boolean; certificate_verified_by?: string | null }, idx) => {
                const statusConfig = getStatusConfig(leave.status);
                const StatusIcon = statusConfig.icon;
                    ...
                    <div className="flex flex-col md:flex-row gap-6 relative z-10">
                      ...
                      <div className="flex flex-row md:flex-col gap-2 md:min-w-[140px] justify-center md:border-l border-slate-200/60 md:pl-6">
                        {leave.status === 'Pending Level' && (
                          <>
                            <Button className="w-full rounded-xl bg-orange-600 hover:bg-orange-700 shadow-sm" onClick={() => handleAction(approveLeaveAction, leave.id as string)}>
                              Approve
                            </Button>
                            <Button variant="outline" className="w-full rounded-xl text-red-600 hover:text-red-700 hover:bg-red-50 border-red-200" onClick={async () => {
                              ...
                              if (reason) handleAction(rejectLeaveAction, leave.id as string, reason);
                            }}>
                              Reject
                            </Button>
                          </>
                        ...
                        {leave.leave_type === 'Sick Leave' && leave.status !== 'Cancelled' && leave.status !== 'Rejected' && leave.medical_certificate_url && (isHR || isSuperAdmin) && (
                          <div className="w-full relative">
                            <Select
                              value=""
                              onValueChange={(val) => {
                                if (val === 'view') handleViewMedicalCertificate(leave.id as string);
                                else if (val === 'delete') handleAction(deleteMedicalCertificateAction, leave.id as string);
                                else if (val === 'valid') handleAction(verifyMedicalCertificateAction, leave.id as string);
                                else if (val === 'invalid') handleAction(rejectMedicalCertificateAction, leave.id as string);
                              }}
                        ...
                        {leave.status === 'Approved' && isHR && (
                          <Button variant="outline" className="w-full rounded-xl text-red-600 hover:text-red-700 hover:bg-red-50 border-red-200" onClick={() => handleAction(cancelApprovedLeaveAction, leave.id as string)}>
                            Cancel Leave
                          </Button>
                        )}
