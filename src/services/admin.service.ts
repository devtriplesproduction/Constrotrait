...
export async function toggleUserActive(userId: string, isActive: boolean, authUser?: User) {
  try {
    const currentUser = authUser || await getAuthenticatedUser();
...
export async function offboardEmployee(userId: string, authUser?: User) {
  try {
    const currentUser = authUser || await getAuthenticatedUser();
    if (!currentUser) return { success: false, error: "Unauthorized" };
...
export async function onboardEmployee(userId: string, authUser?: User) {
  try {
    const currentUser = authUser || await getAuthenticatedUser();
    if (!currentUser) return { success: false, error: "Unauthorized" };
...
export async function deleteEmployee(userId: string, authUser?: User) {
  try {
    const currentUser = authUser || await getAuthenticatedUser();
    if (!currentUser) return { success: false, error: "Unauthorized" };
