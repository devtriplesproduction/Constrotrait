...
export async function isWorkingDayForEmployee(employeeId: string, targetDateStr: string): Promise<boolean> {
  ...
  // The DB query doesn't select date because it's filtered, but the resolver needs it.
  const activeHolidays = (holidays || []).map(h => ({ ...h, date: datePart }));

  return resolveWorkingDay(targetDateStr, profile, activeHolidays);
}
