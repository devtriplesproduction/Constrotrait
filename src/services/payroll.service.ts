...
import { isHolidayForEmployee, resolveWorkingDay } from "@/lib/holiday-utils";
...
export async function calculateMonthlyPayroll(month: number, year: number, branchId: string): Promise<{ isLocked: boolean; cycle: PayrollCycle | null; data: PayrollSnapshot[]; appliedAdjustments?: { ledger_id: string; adjustment_type: string; adjustment_category: string | null; amount: number }[] }> {
  ...
  for (const emp of employees) {
    ...
    for (let day = 1; day <= lastDay; day++) {
      ...
      const isWorking = resolveWorkingDay(currentDateStr, emp, allHolidays || []);
      const isHoliday = isHolidayForEmployee(currentDateStr, emp, allHolidays || []);
    ...
    for (const entry of empLedger) {
        ...
        // TDS is included so that it gets processed and marked as completed by the lock cycle
    ...
    // TDS formula is not documented, using manual amount from ledger
