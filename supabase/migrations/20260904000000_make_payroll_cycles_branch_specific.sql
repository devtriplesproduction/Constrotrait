-- Add branch_id to payroll_cycles
ALTER TABLE public.payroll_cycles ADD COLUMN branch_id UUID REFERENCES public.branches(id) ON DELETE CASCADE;

-- Populate existing records with a default branch_id to avoid NOT NULL constraint failure, if data exists
DO $$ 
DECLARE
...
BEGIN
    ...
    IF default_branch_id IS NOT NULL THEN
        UPDATE public.payroll_cycles SET branch_id = default_branch_id WHERE branch_id IS NULL;
    END IF;
END $$;

-- Drop the old unique constraint
ALTER TABLE public.payroll_cycles DROP CONSTRAINT IF EXISTS payroll_cycles_month_year_key;

-- Create the new unique constraint for branch independence
ALTER TABLE public.payroll_cycles ADD CONSTRAINT payroll_cycles_branch_month_year_key UNIQUE (branch_id, month, year);

-- Drop old index and create new one
DROP INDEX IF EXISTS idx_payroll_cycles_month_year;
CREATE INDEX IF NOT EXISTS idx_payroll_cycles_branch_month_year ON public.payroll_cycles(branch_id, month, year);

-- Update lock_payroll_cycle RPC to consume branch_id
